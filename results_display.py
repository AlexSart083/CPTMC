"""
Enhanced results display components with CORRECTED REAL withdrawal support and detailed withdrawal analysis
FIXED VERSION - Corretti tutti i calcoli dei prelievi
"""
...
class ResultsDisplay:
    """Enhanced display of simulation results with CORRECTED REAL withdrawal analysis"""
    ...
    def show_results(results, simulator, total_deposited, n_simulations, years_to_retirement, 
        ...
        """Display simulation results with CORRECTED REAL withdrawal analysis"""
        st.markdown("---")
        ...
        # Display detailed withdrawal analysis
            ...
            with col1:
                ...
                try:
                    ...
                    if np.isnan(correlation_acc_final):
                        st.info("**Correlazione**: Non calcolabile (varianza zero)" if lang == 'it' else "**Correlation**: Cannot be calculated (zero variance)")
                    else:
                        st.info(f"**{'Correlazione' if lang == 'it' else 'Correlation'}**: {correlation_acc_final:.3f}")
                except:
                    st.info("**Correlazione**: Non calcolabile" if lang == 'it' else "**Correlation**: Cannot be calculated")
        ...
        with tab3:
            ...
            with col2:
                ...
                if valid_pairs:
                    ...
                    st.info(f"""
                    **{'Impatto Inflazione Media' if lang == 'it' else 'Average Inflation Impact'}**: {inflation_impact:.1f}%
                    
                    {"I valori reali sono mediamente il" if lang == 'it' else "Real values are on average"} {avg_real_percentage:.1f}% {"dei valori nominali" if lang == 'it' else "of nominal values"}
                    
                    *{"Calcolato su" if lang == 'it' else "Calculated on"} {len(valid_pairs)} {"scenari con portafoglio residuo > 0" if lang == 'it' else "scenarios with remaining portfolio > 0"}*
                    """)
                else:
                    st.warning("""
                    **⚠️ {"Attenzione" if lang == 'it' else "Warning"}**: {"Tutti gli scenari hanno portafoglio esaurito (valore finale = 0)" if lang == 'it' else "All scenarios have depleted portfolio (final value = 0)"}
                    
                    {"Impossibile calcolare l'impatto inflazione" if lang == 'it' else "Cannot calculate inflation impact"}
                    """)
